---
title: "QM 2025 Week 3: Statistical Inference"
...
```{r example sampling distribution}

ages <- c(29, 25, 24, 19, 20,
          21, 18, 30, 22, 19, 
          18, 25, 21, 18, 23,
          19, 28)

hist(ages, breaks = 5, freq = F)
plot(density(ages, kernel = "gaussian", adjust = 0.5))
...
sample_ages <- sample(ages, 8)

mean_sample <- mean(sample_ages)
mean_sample
mean_ages
...
mean_samples <- rep(NA,1000)

for (i in 1:1000){
  sample_ages <- sample(ages,8)
  mean_samples[i] <- mean(sample_ages)
}

hist(mean_samples, breaks = 6, freq =F)
lines(density(mean_samples))
abline(v = mean_ages, col = "blue",  lwd =2)
...
se_sample <- sd(sample_ages)/sqrt(length(sample_ages))

ci_sample <- c(mean_sample + qnorm(2.5/100)*se_sample, mean_sample + qnorm(97.5/100)*se_sample)


#plot(seq(-3,3,0.1), dnorm(seq(-3,3,0.1)), type = "l", lwd =2)


if(mean_ages < ci_sample[1] | mean_ages > ci_sample[2]){
  print(i)
  break}
...
plot(mean_sample,1, pch = 19, col = "blue", yaxt = "n", ylab = "",
     xlim = c(10, 30))
lines(ci_sample,c(1,1), col = "blue")
abline(v = mean_ages, col = "red")
...
load(file = "raw-data/support.RData")
...
# 
nsim <-1000
d_west <- rnorm(nsim, mean(west), se_west)
d_east <- rnorm(nsim, mean(east), se_east)

# 2. Calculate quantiles to get CI

ci_west <- quantile(d_west, c(0.025, 0.975))
ci_east <- quantile(d_east, c(0.025, 0.975))
...
# # 2. Calculate quantiles to get CI
# 
 ci_sim <- quantile(diff, c(0.025, 0.975))
# 
# # 3. plot and mean
# 
hist(diff) 
